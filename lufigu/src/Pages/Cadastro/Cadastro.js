...
const Cadastro = ({ name, email, password, confirmPassword, setName, setEmail, setPassword, setConfirmPassword }) => {
  ...
  const agoraCadastro = async () => {
    if (!name || !email || !password || !confirmPassword) {
      ...
      setInputs(false);
      try {
        await axios.post('https://lufigu.onrender.com/cadastro', { name, email, password, confirmPassword });
  ...
  const responseGoogle = async (response) => {
    try {
      ...
      await axios.post('https://lufigu.onrender.com/cadastro', { name, email, isGoogleUser: true });
